...
class SoodVzianTafziliScreen extends StatelessWidget {
  const SoodVzianTafziliScreen({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("تفضیلی سود و زیان"),
        backgroundColor: Color(AppColor.primaryColor),
      ),
      body:OrientationBuilder(builder:(context, orientation) => orientation == Orientation.portrait ? _portraitView(wi , he , Get.find<SoodVzianDetailController>() ) : _landView(wi , he , Get.find<SoodVzianDetailController>())) ,);

  }
  Widget _portraitView(double wi , double he , SoodVzianDetailController lv3controller ){
    wi = Get.width;
    he = Get.height;
    var listTaraz = lv3controller.tafziliListModel.value.tarazAzmayeshiKolMoeinTafsiliTafsiliList;
  ...
  Widget _landView(double wi , double he , SoodVzianDetailController lv3controller){
    var listTaraz = lv3controller.tafziliListModel.value.tarazAzmayeshiKolMoeinTafsiliTafsiliList;
    return Container(
      width: Get.width,
      height: Get.height,
